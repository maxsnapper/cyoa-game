        ...
        for room1 in self.rooms:
            for room2 in self.rooms:
                ...
                if ((room1.y2 == room2.y) or 
                    (room1.y == room2.y2)):
                    if room1.y2 == room2.y: y = room1.y2
                    else: y = room1.y
                    ...
                    if x :
                        ...
                        new_door = Doors(self.game, x, y, (room1, room2), len(self.doors))
                        if not (new_door.is_duplicate(self.game, self.doors)):
...
class Doors(object):
    def __init__(self, game, x, y, rooms, door_id=0):
    	self.game = game
        self.x = x - 10
        self.y = y - 10
        self.width = 20
    ...
    def is_duplicate(self, game, doors):
        is_duplicate = False
        for door in doors:
